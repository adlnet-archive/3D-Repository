...
NOTE: MYSQL table create definition 
CREATE TABLE users (
...
namespace Simple.Providers.MySQL
{
    ...
    public sealed class MysqlMembershipProvider : MembershipProvider
    {
        ...
        private string eventSource = "SimpleMysqlMembershipProvider";
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            if (name == null || name.Length == 0)
                name = "SimpleMysqlMembershipProvider";

            if (String.IsNullOrEmpty(config["description"]))
            {
                config.Remove("description");
                config.Add("description", "Simple MySQL Membership provider");
            }
                 ...
                 out MembershipCreateStatus status)
        {
            ...
            if (u == null)
            {
                ...
                if (pApplicationName == null) pApplicationName = "";
                ...
                cmd.Parameters.Add("@IsApproved", OdbcType.TinyInt).Value = isApproved;
        ...
        public override int GetNumberOfUsersOnline()
        {
            ...
            try
            {
                ...
                numOnline = int.Parse(cmd.ExecuteScalar().ToString());
            }
            catch (OdbcException e)
            {
                if (WriteExceptionsToEventLog)
        ...
        public override string GetUserNameByEmail(string email)
        {
            ...
            try
            {
                ...
                catch (NullReferenceException nre)
                {
                    if (WriteExceptionsToEventLog)
                    {
                        WriteToEventLog(nre, "GetUsernameByEmail");
                    }
                }
            }
            catch (OdbcException e)
        ...
        public override bool ValidateUser(string username, string password)
        {
            ...
            OdbcCommand cmd = new OdbcCommand("SELECT Password, IsApproved FROM `" + tableName + "`" +
                    " WHERE Username = '"+username+"' AND ApplicationName = '"+pApplicationName+"' AND IsLockedOut = 0", conn);
            ...
            try
            {
                ...
                if (CheckPassword(password, pwd))
                {
                    if (isApproved)
                    {
                        ...
                        OdbcCommand updateCmd = new OdbcCommand("UPDATE `" + tableName + "` SET LastLoginDate = NOW()" +
            ...
            try
            {
                ...
                else
                {
                    if (failureCount++ >= MaxInvalidPasswordAttempts)
                    {
                        ...
                        cmd.CommandText = "UPDATE `" + tableName + "` " +
                                          "  SET IsLockedOut = 1, LastLockedOutDate = NOW() " +
                                          "  WHERE Username = '"+username+"' AND ApplicationName = '"+ApplicationName+"'";
