...
function Texture_Load_Callback(texturename) {
    ...
    var url = WebGL.gURL.substr(0, WebGL.gURL.length - 34);
    url = url + "Textures/NoRedirect/" + texturename
   
    if (false && !ThumbExists(texturename)) {
        
        
        var newthumb = new Thumbnail(url, texturename);
            WebGL.ThumbNails.push(newthumb);
            newthumb.attachTo("canvas_Wrapper");
            newthumb.SetPosition(5, WebGL.ThumbNails.length * 60 - 30);
	}
        var t = osg.Texture.create(url);
    t.file = texturename;
