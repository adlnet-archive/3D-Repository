	...
	mousemove : function(ev) {
	    ...
	    UpdateCamera();
		  ...
		  //o.accept(new PrepareForExportVisitor());
		    ...
		    var pos = [0,0,1];
		    pos[0] = (((WebGL.MouseX/1280)) - .5 ) *2 ;
		    pos[1] = (((WebGL.MouseY/1024))  - .5 )*2;
		    
		    var inverseview = osg.Matrix.inverse(osg.Matrix.mult(WebGL.gCamera.getProjectionMatrix(),WebGL.gCamera.getViewMatrix()));
		    pos = osg.Matrix.transformVec3(inverseview,pos);
		    var dir = osg.Vec3.sub(osg.Vec3.add(WebGL.gCameraTarget,WebGL.gCameraOffset),pos);
		    dir = osg.Vec3.normalize(dir);
	          //  osg.Matrix.getTrans(gVehicle.matrix,pos);
		  
		 //holy crap this worked out really well
		    //ray/heighfield intersection
		    var point2 = osg.Vec3.add(WebGL.gCameraTarget,WebGL.gCameraOffset);
		    var point1 = osg.Vec3.add(point2,dir);
		    var tmax = (0-point1[1])/(point2[1] - point1[1]);
		    var tmin = (100-point1[1])/(point2[1] - point1[1]);
		    var ti = tmax;
		    var yi = 0;
		    var steps = 100;
		    for(var k = 0; k< 3; k++)
			{
        		    for(var i = tmin; i<tmax; i+=(tmax-tmin)/steps)
        		    {
        			var y0 =  point1[1] +(point2[1]-point1[1])*i;
        			var x = point1[0] +(point2[0]-point1[0])*i;
        		    	var z = point1[2] +(point2[2]-point1[2])*i;
        		    	pos = [x,0,z];
        		    	pos = toHeightMapSpace(pos);
        		    	var y = SampleHeightmap(pos)[1]/2;
        		    	if(y>y0 )
        		    	{
        		    	    ti = i;
        		    	    yi = y;
        		    	    tmax = i;
        		    	    tmin = i-(tmax-tmin)/steps;
        		    	    break;
        		    	}   
        		    }
			}
		  
		    var x = point1[0] +(point2[0]-point1[0])*ti;
		    var z = point1[2] +(point2[2]-point1[2])*ti;
		    pos = [x,0,z];
		   // alert(pos);
		    pos = toHeightMapSpace(pos);
		    
		    if(WebGL.gPaintPositionUniform)
		    WebGL.gPaintPositionUniform.set([pos[0]/512,1-pos[2]/512,brushsize]);
		    
		   
		    
		    if(WebGL.MouseMode == "select" && WebGL.gMouseDown)
		    {  
		    var gradient = _2dctx.createRadialGradient(pos[0],pos[2], 0, pos[0],pos[2], brushsize/2);
		    if(brushmode=="pull")
			{
		    ...
		    _2dctx.fillRect(pos[0]-brushsize/2,pos[2]-brushsize/2,brushsize,brushsize);
		    if(landscapeTexture)
...
function GetDepthShader() {
    ...
    var vertshader = [
	    ...
	    "",
	    "void main() {",
	    "float z = texture2D(heightmap,TexCoord0).z * 50.0;",
...
function GetLandscapeShader() {
    ...
    var vertshader = [
	    ...
	    "vec3 leftvert = vec3(Vertex.x + 1.0*(200.0/512.0),texture2D(heightmap,TexCoord0 + vec2(1.0/512.0,0)).z * 50.0 + Vertex.y,Vertex.z);",
	    "vec3 frontvert = vec3(Vertex.x,texture2D(heightmap,TexCoord0+ vec2(0,1.0/512.0)).z * 50.0 + Vertex.y,Vertex.z+ 1.0*(200.0/512.0));",
	    ...
	    //"Normal = normalize(Normal);",
	    "oWorldNormal = norm;",
	    "oNormal = normalize((NormalMatrix * vec4(norm,1.0)).xyz);",
	    "oTC0 = TexCoord0;",
	    "oViewSpaceVertex = (ModelViewMatrix * vert).xyz;",
	    "mat4 ModelMatrix =  inverseViewMatrix * ModelViewMatrix;",
	    "oWorldPos = vert.xyz;",
...
//COMMENT OUT ABOVE RETURNS FOR HIGHER QUALITY SHADOWS
	  ...
	  "  }",
	    ...
	    "vec4 rock = SRGB_to_Linear(texture2D(rockmap,oTC0* 10.0));",
	    "vec4 snow = (texture2D(snowmap,oTC0* 1.0));",
	    "vec4 dirt = SRGB_to_Linear(texture2D(dirtmap,oTC0* 10.0));",
	    "vec4 water = SRGB_to_Linear(texture2D(watermap,oTC0 * 10.0));",
	    "vec4 grass = SRGB_to_Linear(texture2D(grassmap,oTC0 * 10.0));",
	    "vec4 mix = vec4(0.0,0.0,0.0,0.0);",
	    "float watermix = 0.0;",
	    
	    
	    "if(oWorldPos.y > 25.0) mix[1] = clamp((oWorldPos.y - 25.0)/3.0,0.0,1.0);",
	    "if(oWorldPos.y > 1.0 ) mix[2] = 1.0 - mix[1];",
	    "if(oWorldPos.y < 1.0) watermix = 1.0;",
	    "mix[0] = (1.0-dot(oWorldNormal,vec3(0.0,1.0,0.0))) * 10.0;",
	    "mix = normalize(mix);",
	    "vec4 ret = (rock*mix[0]+snow*mix[1]+grass*mix[2]+dirt*mix[3])*(1.0-watermix) + water * watermix;",
	    "ret.a = 1.0;",
	    "return ret;",
	    "}",
	    
	    "void main() {",
	 //   "gl_FragColor = vec4( oLightSpaceNormal,1.0);",
	 //   "return;",
	    "if(InWireframe == 1) {gl_FragColor = vec4(0.0,0.0,IsPicked,1.0); return;}",
	    "vec4 oShadowSpaceVertexW = oShadowSpaceVertex / oShadowSpaceVertex.w;",
	    "oShadowSpaceVertexW.xy *= .5;",
	    ...
	    "shadow = getShadowColor(oShadowSpaceVertexW,oScreenPosition,canvasSize.y,canvasSize.x,shadowmap);",
	   // "shadow = mix(shadow,1.0,length(oShadowSpaceVertex.xy));",
	    "	float NdotL = dot(normalize(oLightSpaceNormal),normalize(oLightDir));",
	    
	    "vec4 diffusetexture = vec4(0.0,0.0,0.0,0.0);",
	    "if(RenderOptions[0] == 1.0)",
	        "diffusetexture = SRGB_to_Linear(texture2D(heightmap,oTC0)) + MaterialDiffuseColor;",
	    ...
	    "gl_FragColor = mix(gl_FragColor,vec4(1.0,1.0,1.0,1.0),clamp(pow((oScreenPosition.z/oScreenPosition.w),42.0),0.0,1.0));",
	    "gl_FragColor.a = diffusetexture.a;",
	    "if(IsPicked == 1) gl_FragColor = mix(gl_FragColor,vec4(0.0,0.0,1.0,1.0),.35);",
	    "if(length(oTC0-PaintPosition.xy) < PaintPosition.z/(512.0*4.0)) gl_FragColor = mix(gl_FragColor,vec4(0.0,0.0,1.0,1.0),.35); ",
	     "}" ].join('\n');
...
function SetupTerrainHeight()
{
    ...
    landscapecanvas.style = "position:absolute";
    ...
    _2dctx.fillRect(0,0,512,512);
    heightmap.onload = function()
    {
	//_2dctx.drawImage(heightmap,0,0);
	imageData = _2dctx.getImageData(0,0,512,512);
	document.getElementById('HeightmapPreview').appendChild(landscapecanvas); 
...
*/	
	...
	// _2dctx.drawImage(heightmap,0,0);
	 ...
	 WebGL.gModelRoot.addChild(gLandscape);
	    ...
	    newstateset.addUniform(osg.Uniform.createInt1(1,"grassmap"));
	    newstateset.addUniform(osg.Uniform.createInt1(4,"rockmap"));
	    newstateset.addUniform(osg.Uniform.createInt1(5,"snowmap"));
	    newstateset.addUniform(osg.Uniform.createInt1(6,"watermap"));
	    newstateset.addUniform(osg.Uniform.createInt1(7,"dirtmap"));

	    var grasstex = osg.Texture.create("file:///C:\\Development\\HTML5Game\\Assets\\Textures\\grass.jpg");
	    newstateset.setTexture(1, grasstex);
	    var rocktex = osg.Texture.create("file:///C:\\Development\\HTML5Game\\Assets\\Textures\\rock.png");
	    newstateset.setTexture(4, rocktex);
	    var snowtex = osg.Texture.create("file:///C:\\Development\\HTML5Game\\Assets\\Textures\\snow.jpg");
	    newstateset.setTexture(5, snowtex);
	    var watertex = osg.Texture.create("file:///C:\\Development\\HTML5Game\\Assets\\Textures\\water.jpg");
	    newstateset.setTexture(6, watertex);
	    var dirttex = osg.Texture.create("file:///C:\\Development\\HTML5Game\\Assets\\Textures\\dirt.jpg");
	    newstateset.setTexture(7, dirttex);

	    WebGL.gPaintPositionUniform = osg.Uniform.createFloat3([ 0,0,10 ], 'PaintPosition');
	    
	    newstateset.addUniform(WebGL.gPaintPositionUniform)
	      
	    WebGL.gRenderOptionsUniform =  osg.Uniform.createFloat4([ 0,0,0,10 ], 'RenderOptions');
...
function SampleHeightmap(input)
{
	...
	var sample1 = sample.data[4*0]/255;
	var sample2 = sample.data[4*1]/255;
	var sample3 = sample.data[4*2]/255;
	var sample4 = sample.data[4*3]/255;
	
	    
    var filtered = (sample1   * u_opposite  + sample2   * u_ratio) * v_opposite + 
    (sample3 * u_opposite  + sample4 * u_ratio) * v_ratio;
    
	var newheight = (filtered)*100;
