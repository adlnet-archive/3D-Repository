...
var gviewer;
var gSceneRoot;
var gWireframeButton;
var gUpButton;
var gRadius = 1;
var gCameraOffset = [ gRadius, 0, 0 ];
var gCameraTarget = [ 0, 0, 0 ];
var gUpVector = [ 0, 1, 0 ];
var gCameraGoal = gCameraOffset;
...
var gAnimatingRotation = false;
var gCanvasSizeUniform;
...
function BuildGridGeometry() {
    ...
    var QuadSizeX = gSceneBounds.GetRadius() * 10;
    var QuadSizeY = gSceneBounds.GetRadius() * 10;
    var rq;
    if (GetUpVector() == "Z") {
	rq = osg.createTexuredQuad(QuadSizeX / 2.0, 0, -QuadSizeY / 2.0,
		-QuadSizeX, 0, 0, 0, 0, QuadSizeY, gSceneBounds.GetRadius(),
		gSceneBounds.GetRadius(), 0, 0);
    } else {
	rq = osg.createTexuredQuad(-QuadSizeX / 2.0, -QuadSizeY / 2.0, 0,
		QuadSizeX, 0, 0, 0, QuadSizeY, 0, gSceneBounds.GetRadius(),
		gSceneBounds.GetRadius(), 0, 0);
    }
    ...
    n.addChild(rq);
    // alert([gSceneBounds.GetCenter()[0],gSceneBounds.GetCenter()[1],gSceneBounds.GetCenter()[2]]);
    if (GetUpVector() == "Y") {
	n.matrix = osg.Matrix.makeTranslate(gSceneBounds.GetCenter()[0],
		gSceneBounds.GetCenter()[1], gSceneBounds.GetMin()[2]);
    } else
	n.matrix = osg.Matrix.makeTranslate(gSceneBounds.GetCenter()[0],
		gSceneBounds.GetMin()[1], gSceneBounds.GetCenter()[2]);
    return n;
...
function GetUpVector() {
    if (gUpVector[0] == 0 && gUpVector[1] == 1 && gUpVector[2] == 0)
	return "Z";
    else if (gUpVector[0] == 0 && gUpVector[1] == 0 && gUpVector[2] == 1)
	return "Y";
}
function SetUpY() {
    gUpVector = [ 0, 1, 0 ];
...
function SwapUpVector() {
    ...
    gModelRoot.setMatrix(osg.Matrix.makeIdentity());
    if (gUpVector[0] == 0 && gUpVector[1] == 1 && gUpVector[2] == 0)
	SetUpZ();
    else if (gUpVector[0] == 0 && gUpVector[1] == 0 && gUpVector[2] == 1)
	SetUpY();
...
function RotateCamera(x, y) {
    var r = osg.Matrix.makeRotate(x / 100.0, gUpVector[0], gUpVector[1],
	    gUpVector[2]);
    var cross = osg.Vec3.cross(gUpVector, gCameraOffset);
 ...
 //    the rotation matrix
    gCameraOffset = osg.Matrix.transformVec3(osg.Matrix
	    .inverse(r2),gCameraOffset);
...
function UpdateCamera() {
    gCamera.setViewMatrix(osg.Matrix.makeLookAt(osg.Vec3.add(
	    gCameraOffset, gCameraTarget), gCameraTarget, gUpVector));
   
    if(g_RTT)
	{
	    g_RTT.setViewMatrix(osg.Matrix.makeLookAt(osg.Vec3.add(
	    [gSceneBounds.GetRadius()*1.5,gSceneBounds.GetRadius()*1.5,gSceneBounds.GetRadius()*1.5], gCameraTarget), gCameraTarget, gUpVector));
	
	    //g_RTT.setViewMatrix(gCamera.getViewMatrix());
	   // g_RTT.setProjectionMatrix(gCamera.getProjectionMatrix());
	
	    g_RTT.setProjectionMatrix(osg.Matrix.makeOrtho(-gSceneBounds.GetRadius()*1, gSceneBounds.GetRadius()*1, -gSceneBounds.GetRadius()*1, gSceneBounds.GetRadius()*1, .01, 10000.0));
	    
	    if(ViewMatrixUniform)
		ViewMatrixUniform.set(osg.Matrix.inverse(gCamera.getViewMatrix()));
	    //else(alert("wtf"));
	    if(ShadowMatrixUniform)
		ShadowMatrixUniform.set(osg.Matrix.mult(g_RTT.getProjectionMatrix(),g_RTT.getViewMatrix()));
		//ShadowMatrixUniform.set(osg.Matrix.mult(g_RTT.getProjectionMatrix(),g_RTT.getViewMatrix()));
	    
	  //  alert([gviewer.canvas.clientWidth,gviewer.canvas.clientHeight]);
	    if(gCanvasSizeUniform)
		gCanvasSizeUniform.set([gviewer.canvas.clientWidth,gviewer.canvas.clientHeight]);
	    
	}
}
function BindInputs() {

    jQuery(gviewer.canvas).bind({
	...
	if (jQuery(document).mousewheel !== undefined) {
	    jQuery(document).mousewheel(
			...
			if (intDelta > 0) {
			    ...
			    gCameraOffset = osg.Vec3.mult(gCameraOffset, .9);
			} else if (intDelta < 0) {
			    gCameraOffset = osg.Vec3.mult(gCameraOffset, 1.1);
			}
			return false;
		    });
	}
...
function CreateButtons() {
    ...
    CreateButton("../../../../Images/Icons/3dr_btn_R_cube.png",
	    ...
	    AnimateLeft);
    gUpButton = CreateButton("../../../../Images/Icons/3dr_btn_Y.png",
	    "../../../../Images/Icons/3dr_btn_grey_Y.png", 0, 0, 3, SwapUpVector);
    gWireframeButton = CreateButton(
...
{
    gAnimatingRotation = gAnimatingRotation == false;
    gModelRoot.setMatrix(osg.Matrix.makeIdentity());
}
function initWebGL(location, upaxis, scale, showscreenshot) {
...
function Texture_Load_Callback(texturename) {
    ...
    return osg.Texture.create(gURL + "&Texture=" + texturename);
};
function AnimateTop() {
    gAnimating = true;
    //   alert(GetUpVector() === "Z");
    if (GetUpVector() === "Y")
	gCameraGoal = [ 0, 0, osg.Vec3.length(gCameraOffset) ];
    else
	gCameraGoal = [ 0, osg.Vec3.length(gCameraOffset), 0 ];
}
function AnimateFront() {
    gAnimating = true;
    gCameraGoal = [ osg.Vec3.length(gCameraOffset), 0, 0 ];
}
...
AnimationCallback.prototype = {
    update : function() {
	    ...
	    if(gModelRoot && gAnimatingRotation == true)
		{
		gModelRoot.setMatrix(osg.Matrix.mult(gModelRoot.getMatrix(),osg.Matrix.makeRotate(.005, gUpVector[0], gUpVector[1], gUpVector[2])));
		
		}
	    
	if (gAnimating) {
	    var tempoffset = [ 0, 0, 0 ];
	    osg.Vec3.copy(gCameraOffset, tempoffset);
	    tempoffset = osg.Vec3.normalize(tempoffset);

	    var tempgoal = [ 0, 0, 0 ];
	    osg.Vec3.copy(gCameraGoal, tempgoal);
	    tempgoal = osg.Vec3.normalize(tempgoal);

	    var dot = osg.Vec3.dot(tempoffset, tempgoal);
	    if (dot > .99
		    && osg.Vec3
			    .length(osg.Vec3.sub(gCameraOffset, gCameraGoal)) < .1)
		gAnimating = false;
	    else {
		gCameraOffset = osg.Vec3.lerp(.02, gCameraOffset, gCameraGoal);
		UpdateCamera();
	    }
	}
    }
...
function RebuildGrid() {
    if (gGridNode)
	gSceneRoot.removeChild(gGridNode);
    gGridNode = BuildGridGeometry();
    gSceneRoot.addChild(gGridNode);
}
...
function GetRecieveShadows()
{
    ...
    var vertshader = [
                     ...
                     //"oLightSpaceNormal = normalize((shadowProjection * vec4(Normal, 1.0)).xyz);",
    ...
    var fragshader = [
                      ...
                      //the 512 here should be the shadowmap resolution!
                          "float shadowz = unpackFloatFromVec4i(texture2D(map,shadowCoord.xy + offset * (1.0/1024.0)));",
                          "float near = 1.0;",
                          "float far = 10.0;",
                      ...
                      "	vec2 offset = vec2(0.0,0.0);",
                      	...
                      	//these numbers 400 and 300 should be 1/2 the screen height and width
                    ...
                    //  "return (x1 + x2 + x3 + x4) / 4.0;",
                      ...
                      "	float NdotL = dot(oLightSpaceNormal,oLightDir);",
                   
                      "gl_FragColor =  min(clamp(shadow,0.3,1.0),clamp(NdotL+.3,.3,1.0))*1.2 * texture2D(texture,oTC0);",
...
function onJSONLoaded(data) {
    ...
    gviewer.canvas.width = gviewer.canvas.clientWidth;
    gviewer.canvas.height = gviewer.canvas.clientHeight;

   // alert(JSON.stringify(data));
    o = ParseSceneGraph(data,Texture_Load_Callback);
 
    
    gCamera = new osg.Camera();
    
    
    gCamera.setRenderOrder(osg.Camera.NESTED_RENDER, 0);
    gCamera.setReferenceFrame(osg.Transform.ABSOLUTE_RF);
    gCamera.setViewport(new osg.Viewport(0,0,gviewer.canvas.clientWidth,gviewer.canvas.clientHeight));
    
    gModelRoot = osg.MatrixTransform.create();
    gModelRoot.addChild(o);
    var bv = new BoundsVisitor();

    
    gCamera.addChild(gModelRoot);
    gCamera.accept(new AmbientVisitor([ .5, .5, .5, 1 ]));

    var ratio = gviewer.canvas.clientWidth / gviewer.canvas.clientHeight;

    gSceneRoot = gCamera;

    bv.apply(gModelRoot);
    gSceneBounds = bv.asBoundingBox();
  
    gCamera.addChild(CreateBoundsGeometry(gSceneBounds.points));
    var radius = gSceneBounds.GetRadius();

    gCameraOffset = [ radius, radius, radius ];
    gCameraTarget = gSceneBounds.GetCenter();
    UpdateCamera();
    ...
    gCamera.addChild(gGridNode);
    gCamera.getOrCreateStateSet().setAttribute(
	    new osg.BlendFunc("ONE", "ONE_MINUS_SRC_ALPHA"));
    
    
    var CameraTexturePair = BuildShadowCamera();
    
    g_RTT = CameraTexturePair.camera;
   // gCamera.addChild(CameraTexturePair.camera);
    CameraTexturePair.camera.addChild(gModelRoot);
    CameraTexturePair.camera.setViewMatrix(osg.Matrix.makeLookAt(gCameraOffset,
	    gCameraTarget, gUpVector));
    
   
//    var rq = osg.createTexuredQuad(10 / 2.0, .001, -10 / 2.0,
//		-10, 0, 0, 0, 0, 10, gSceneBounds.GetRadius(),
//		gSceneBounds.GetRadius(), 0, 0);
//    rq.getOrCreateStateSet().setTextureAttribute(0,CameraTexturePair.texture);
    
    gCamera.getOrCreateStateSet().setAttribute(GetRecieveShadows());
    gCamera.getOrCreateStateSet().setTextureAttribute(3,CameraTexturePair.texture);
    gCamera.getOrCreateStateSet().addUniform(osg.Uniform.createInt1(3,'shadowmap'));
    gCamera.getOrCreateStateSet().addUniform(osg.Uniform.createInt1(0,'texture'));
    gCanvasSizeUniform = osg.Uniform.createFloat2([gviewer.canvas.clientWidth,gviewer.canvas.clientHeight],'canvasSize');
    gCamera.getOrCreateStateSet().addUniform(gCanvasSizeUniform);
    UpdateCamera();
    
    
    ViewMatrixUniform = osg.Uniform.createMatrix4(osg.Matrix.inverse(gCamera.getViewMatrix()),'inverseViewMatrix');
    ShadowMatrixUniform = osg.Uniform.createMatrix4(osg.Matrix.mult(g_RTT.getProjectionMatrix(),g_RTT.getViewMatrix()),'shadowProjection');
    gCamera.getOrCreateStateSet().addUniform(ShadowMatrixUniform);
    gCamera.getOrCreateStateSet().addUniform(ViewMatrixUniform);
  //  gCamera.addChild(rq);

    gviewer.setScene(gCamera);
 
    gCamera.setClearColor([ 1, 1, 1, 1 ]);
    
    gCamera.setViewMatrix(osg.Matrix.makeLookAt(gCameraOffset,
	    gCameraTarget, gUpVector));
    gCamera.setProjectionMatrix(osg.Matrix.makePerspective(60, ratio, .01,
	    10000));
    
   // gCamera.setProjectionMatrix(osg.Matrix.makeOrtho(-10, 10, -10, 10, -0, 10000.0));
    gviewer.view.setClearColor([1,1,1,1]);
    gviewer.view.addChild(CameraTexturePair.camera);
    gviewer.scene.setUpdateCallback(new AnimationCallback);
    gviewer.view.getOrCreateStateSet(new osg.Depth());
    gviewer.run();
...
BoundsVisitor.prototype = osg.objectInehrit(osg.NodeVisitor.prototype, {
	...
	if (node.traverse) {
		...
		// gSceneRoot.addChild(CreateBoundsGeometry(this.currentbounds));
		return this.currentbounds;
...
function ApplyWireframe() {
    gModelRoot.accept(new WireframeVisitor());
    gWireframeButton.onclick = UndoWireframe;
}
function UndoWireframe() {
    gModelRoot.accept(new UnWireframeVisitor());
    gWireframeButton.onclick = ApplyWireframe;
}
...
function ParseSceneGraph(node, texture_load_callback) {
    ...
    if (node.children) {
        ...
        for (var child = 0, childLength = node.children.length; child < childLength; child++) {
            node.children[child] = ParseSceneGraph(node.children[child],texture_load_callback);
        }
    }
